  ...
  switch (type) {
    case "QUESTION":
      if (metadata?.questionText && metadata?.responseValue !== undefined) {
        prefix = `Based on your response of "${metadata.responseValue}" to "${metadata.questionText.substring(0, 40)}${metadata.questionText.length > 40 ? '...' : ''}"`;
            ...
            <div className="space-y-4">
              {latestSuggestions
                ...
                .map((suggestion) => {
                  ...
                  return (
                    <div key={suggestion.id} className="border border-gray-200 rounded-lg p-4 hover:border-cropper-blue-300 transition-all duration-200">
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-2 mb-2">
                            <span className="px-2 py-1 bg-cropper-blue-100 text-cropper-blue-800 rounded-full text-xs font-medium">
                              {category}
                            </span>
                            ...
                            {suggestion.metadata?.isStrategic && (
                              <span className="px-2 py-1 bg-cropper-purple-100 text-cropper-purple-800 rounded-full text-xs font-medium">
                                Strategic
                              </span>
                            )}
                          </div>
