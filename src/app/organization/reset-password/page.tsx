...
function ResetPasswordForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => router.push("/organization/login"), 2000);
    } catch (err) {
  ...
  if (!token) {
    return (
      <div className="rounded-lg bg-red-50 border border-red-200 p-4 text-center">
        ...
        <Link href="/organization/forgot-password" className="mt-2 inline-block text-sm font-medium text-cropper-mint-600">
          Request a new link
        </Link>
      </div>
    );
  ...
  if (success) {
    return (
      <div className="rounded-lg bg-cropper-mint-50 border border-cropper-mint-200 p-4 text-center">
        ...
        <Link href="/organization/login" className="mt-2 inline-block text-sm font-medium text-cropper-mint-600">
          Go to login
        </Link>
      </div>
    );
