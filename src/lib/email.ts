...
function getSmtpConfig() {
  ...
  // Port 587 uses STARTTLS (TLS upgrade); requireTLS ensures encrypted connection
...
interface OrganizationForgotPasswordEmailParams {
  name: string;
  email: string;
  resetUrl: string;
}

interface AdminForgotPasswordEmailParams {
  name: string;
  email: string;
  resetUrl: string;
}
...
/**
 * Send organization forgot-password email (link to set new password)
 */
export async function sendOrganizationForgotPasswordEmail({
  name,
  email,
  resetUrl,
}: OrganizationForgotPasswordEmailParams): Promise<EmailResult> {
  const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Reset Your Password - CSO Self-Assessment Tool</title>
      </head>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background-color: #f8f9fa; padding: 30px; border-radius: 10px; text-align: center;">
          <h1 style="color: #2c5530; margin-bottom: 20px;">Reset Your Password</h1>
          <p style="font-size: 16px; margin-bottom: 30px;">Hi ${name},</p>
          <p style="font-size: 16px; margin-bottom: 30px;">
            We received a request to reset the password for your organization account. Click the button below to set a new password:
          </p>
          <div style="margin: 30px 0;">
            <a href="${resetUrl}" style="background-color: #2c5530; color: white; padding: 15px 30px; text-decoration: none; border-radius: 5px; font-size: 16px; display: inline-block;">Set New Password</a>
          </div>
          <p style="font-size: 14px; color: #666;">If the button doesn't work, copy and paste this link into your browser:<br><a href="${resetUrl}" style="color: #2c5530; word-break: break-all;">${resetUrl}</a></p>
          <p style="font-size: 14px; color: #666; margin-top: 30px;">This link will expire in 1 hour. If you didn't request a password reset, you can ignore this email.</p>
          <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;">
    ...
    `;
  const textContent = `Reset Your Password\n\nHi ${name},\n\nWe received a request to reset the password for your organization account. Visit this link to set a new password:\n${resetUrl}\n\nThis link will expire in 1 hour. If you didn't request a password reset, you can ignore this email.`;
  return sendTemplatedEmail({
    recipient: { email, name },
    subject: 'Reset Your Password - CSO Self-Assessment Tool',
    htmlContent,
    textContent,
    context: 'organization forgot password',
  });
}

/**
 * Send admin forgot-password email (link to set new password)
 */
export async function sendAdminForgotPasswordEmail({
  name,
  email,
  resetUrl,
}: AdminForgotPasswordEmailParams): Promise<EmailResult> {
  const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Reset Admin Password - CSO Self-Assessment Tool</title>
      </head>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background-color: #f8f9fa; padding: 30px; border-radius: 10px; text-align: center;">
          <h1 style="color: #2c5530; margin-bottom: 20px;">Reset Admin Password</h1>
          <p style="font-size: 16px; margin-bottom: 30px;">Hi ${name},</p>
          <p style="font-size: 16px; margin-bottom: 30px;">
    ...
    `;
  const textContent = `Reset Admin Password\n\nHi ${name},\n\nWe received a request to reset the password for your admin account. Visit this link to set a new password:\n${resetUrl}\n\nThis link will expire in 1 hour. If you didn't request a password reset, you can ignore this email.`;
  return sendTemplatedEmail({
    recipient: { email, name },
    subject: 'Reset Admin Password - CSO Self-Assessment Tool',
    htmlContent,
    textContent,
    context: 'admin forgot password',
  });
}
